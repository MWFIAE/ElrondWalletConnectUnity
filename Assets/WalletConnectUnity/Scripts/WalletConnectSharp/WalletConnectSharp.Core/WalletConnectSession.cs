...
namespace WalletConnectSharp.Core
{
    public class WalletConnectSession : WalletConnectProtocol
    {
        ...
        public async Task<string> EthSendTransaction(TransactionData transaction)
        {
            EnsureNotDisconnected();
        ...
        public async Task<string> EthSignTransaction(TransactionData transaction)
        {
            EnsureNotDisconnected();

            var request = new EthSignTransaction(transaction);

            WalletConnectSharp.Core.Models.Ethereum.EthResponse response;
            try
            {
                response = await Send<EthSignTransaction, EthResponse>(request);
                return response.Result;
            }
            catch (IOException e)
            {
        ...
        public async Task<R> Send<T, R>(T data) where T : JsonRpcRequest where R : JsonRpcResponse
        {
            ...
            Events.ListenForResponse<R>(data.ID, (sender, @event) =>
            {
                ...
                Debug.Log("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA SEND response" + response);
                if (response.IsError)
                {
